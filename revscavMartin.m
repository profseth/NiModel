function [A,b,output] = revscavMartin(A,b,do);
%REVSCAVMARTIN scavenges your element downwards by reversible scavenging
...
% The "varb version of this function allows for the distribution of
% POC 'scavenging sites' to have a different distribution than that of POC
% itself, assuming a martin power-law curve set by 'b'

fprintf('%s','revscavMartin...')
...
K = do.revscavMartin.K;
w = do.revscavMartin.w;
bS = do.revscavMartin.bS;
sedremin = do.revscavMartin.sedremin;

% get the POC concentrations from each grid cell
SITES = POC_WJ18;

% Modify the POC concentrations based on the b for scavenging sites
for i=3:24
SITES(:,:,i) = POC_WJ18(:,:,2)*(ao.depth(i)/75)^-bS;
end
